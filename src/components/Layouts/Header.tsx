...
const Header = () => {
    ...
    const getUserDetails = useAppSelector((state) => state.user.auth.bearerToken);
    ...
    const handleSignOut = () => {
        // LoginService.logout(getUserDetails);
        LoginService.logout();
                ...
                <div className="relative bg-white flex w-full items-center px-5 py-2.5 dark:bg-black">
                    <div className="horizontal-logo flex lg:hidden justify-between items-center ltr:mr-2 rtl:ml-2">
                        <Link to="/" className="main-logo flex items-center shrink-0">
                            ...
                            <img className="w-14 ltr:-ml-1 rtl:-mr-1 inline" src={logo} alt="logo" />
                            <span className="text-2xl ltr:ml-1.5 rtl:mr-1.5  font-semibold  align-middle hidden md:inline dark:text-white-light transition-all duration-300">CABRENT</span>
                        </Link>
                        <button
                            type="button"
                    ...
                    <div className="sm:flex-1 ltr:sm:ml-0 ltr:ml-auto sm:rtl:mr-0 rtl:mr-auto flex items-center space-x-1.5 lg:space-x-2 rtl:space-x-reverse dark:text-[#d0d2d6]">
                        ...
                        <div className="dropdown shrink-0">
                            <Dropdown
                                ...
                                button={<img className="w-5 h-5 object-cover rounded-full" src={`/assets/images/flags/IN.svg`} alt="flag" />}
                            >
                                <ul className="!px-2 text-dark dark:text-white-dark font-semibold dark:text-white-light/90 ">
                                    {themeConfig.languageList.map((item: any) => {
                                        return (
                                            <li key={item.code}>
                                                <button
                                                    type="button"
                                                    className={` ${i18next.language === item.code ? 'text-primary' : ''}`}
                                                    onClick={() => {
                        ...
                        {/* <div className="dropdown shrink-0 bg-red-700">
                            <Dropdown
                                offset={[0, 8]}
                                placement={`${isRtl ? 'bottom-start' : 'bottom-end'}`}
                                btnClassName="block p-2 rounded-full bg-white-light/40 dark:bg-dark/40 hover:text-primary hover:bg-white-light/90 dark:hover:bg-dark/60"
                            ...
                            >
                                <ul className="text-dark dark:text-white-dark !py-0 w-[230px] font-semibold dark:text-white-light/90">
                                    ...
                                    <li className="border-t border-white-light dark:border-white-light/10">
                                        <Link to="/login" className="text-danger !py-3" onClick={handleSignOut}>
