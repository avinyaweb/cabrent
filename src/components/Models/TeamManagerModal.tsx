...
interface TeamManagerModalProps {
    event: any; // Assuming it's a button click event
    closeModal: () => void;
    onAddTeamManager: (data: string[], userID: string) => void;
}
...
const TeamManagerModal: React.FC<TeamManagerModalProps> = ({ event, closeModal, onAddTeamManager }) => {
    ...
    useEffect(() => {
        ...
        const fetchAdminData = async () => {
            try {
                ...
                if (data?.admins) {
                    const newData = data.admins.map(({ _id, ...rest }: { _id: any; [key: string]: any }) => ({
                        id: _id,
                        ...rest,
                    }));
    ...
    //     type teamManagerRecord = {
    //         id: string | undefined;
    //         value: string | undefined;
    //     };
    //     try {
    //         const ReportManagerData: teamManagerRecord[] = [];
    //         const userID: string = 'teamManagerData';
    //         for (let i = 0; i < selectedRecords.length; i++) {
    //             const record: teamManagerRecord = {
    //                 id: selectedRecords[i]?.id,
    //                 value: selectedRecords[i]?.firstName,
    //             };
    //             ReportManagerData.push(record);
    //         }
    //         await onAddTeamManager(ReportManagerData, userID);
    //         closeModal();
    //     } catch (error) {
                                    ...
                                    <div className="flex items-center justify-center min-h-screen px-4">
                                        ...
                                        >
                                            <Dialog.Panel className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-5xl my-8 text-black dark:text-white-dark">
                                                <h1 className="text-2xl mt-2 p-5 font-bold text-center">Add Team Manager</h1>
                                                <div>
