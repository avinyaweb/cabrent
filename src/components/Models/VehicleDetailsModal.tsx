...
interface VehicleDetailsModalProps {
    event: any;
    closeModal: () => void;
    onAddVehicle: (serviceCityData: string[], userID: string) => void;
}
...
const VehicleDetailsModal: React.FC<VehicleDetailsModalProps> = ({ event, closeModal, onAddVehicle }) => {
    ...
    //         const userID = 'vehicleAdded';
    //         for (let i = 0; i < selectedRecords.length; i++) {
    //             serviceCityData.push(selectedRecords[i]?.id);
    //         }
    //         await onAddVehicle(serviceCityData, userID);
    //         closeModal();
    //     } catch (error) {
    ...
    useEffect(() => {
        if (selectedCity.trim() !== '') {
            const filteredData = staticVehicleProfileData.filter((item) => item?.fk_serviceCity.toLowerCase() === selectedCity.toLowerCase());
            setVehicleProfileData(filteredData);
        } else {
            setVehicleProfileData(staticVehicleProfileData);
        }
    }, [selectedCity]);
                                            ...
                                            <div>
                                                ...
                                                <div className="p-5">
                                                    <div className="flex justify-end items-center gap-5">
                                                        ...
                                                        <button disabled={selectedRecords.length === 0} type="button" className="btn btn-primary">
                                                            Add
                                                        </button>
                                                    </div>
                                                </div>
