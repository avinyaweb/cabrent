...
import { assignTeamToMultipleAdmin } from '@/services/AdminTeamsService';
import toast from 'react-hot-toast';

interface RisedAgainstModalProps {
    adminTeamsId: any; // Assuming adminTeamsId is a string
    id: string; // Assuming id is a string
    title: string;
}

const RisedAgainstModal: React.FC<RisedAgainstModalProps> = ({ adminTeamsId, id, title }) => {
    interface AdminData {
        id: string;
        firstName: string;
        email: string;
    ...
    //             if (data?.admins) {
    //                 const newData = data.admins.map(({ _id, ...rest }: { _id: string }) => ({
    //                     id: _id,
    //                     ...rest,
    //                 }));
