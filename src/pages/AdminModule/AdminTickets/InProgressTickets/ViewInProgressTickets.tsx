...
interface ViewInProgressTicketsData {
    id: string;
    ticketIdKey: string;
    ticketType: string;
    title: string;
...
const ViewInProgressTickets = () => {
    ...
    const [adminTicketsData, setAdminTicketsData] = useState<ViewInProgressTicketsData[]>(staticTicketData);
    const [page, setPage] = useState(1);
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [pageSize, setPageSize] = useState(PAGE_SIZES[0]);
    const [initialRecords, setInitialRecords] = useState<ViewInProgressTicketsData[]>([]);
    const [recordsData, setRecordsData] = useState<ViewInProgressTicketsData[]>([]);
    const [selectedRecords, setSelectedRecords] = useState<ViewInProgressTicketsData[]>([]);
    ...
    useEffect(() => {
        if (adminTicketsData.length > 0) {
            const sortedData = adminTicketsData.slice().sort((a, b) => {
                const accessor = sortStatus.columnAccessor as keyof ViewInProgressTicketsData;
    ...
    const columns: DataTableColumn<ViewInProgressTicketsData>[] = [
        {
            accessor: 'actions',
            title: 'Actions',
    ...
    const handleRowClick = (row: ViewInProgressTicketsData) => {
            ...
            <ol className="flex text-gray-500 font-semibold dark:text-white-dark flex-wrap">
                ...
                <li
                    className={`before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                        currentPath === '/AdminModule/AdminTickets/InProgressTickets/ViewInProgressTickets' ? 'text-blue-600' : ''
                    }`}
                >
                    InProgress Tickets
                </li>
