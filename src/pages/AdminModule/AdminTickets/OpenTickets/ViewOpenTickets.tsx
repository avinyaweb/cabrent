...
const ViewOpenTickets = () => {
    ...
    const [adminTicketsData, setAdminTicketsData] = useState<ViewOpenTicketsData[]>(staticTicketData);
    const [page, setPage] = useState(1);
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [pageSize, setPageSize] = useState(PAGE_SIZES[0]);
    const [initialRecords, setInitialRecords] = useState<ViewOpenTicketsData[]>([]);
    const [recordsData, setRecordsData] = useState<ViewOpenTicketsData[]>([]);
    const [selectedRecords, setSelectedRecords] = useState<ViewOpenTicketsData[]>([]);
    ...
    useEffect(() => {
        if (adminTicketsData.length > 0) {
            const sortedData = adminTicketsData.slice().sort((a, b) => {
                const accessor = sortStatus.columnAccessor as keyof ViewOpenTicketsData;
    ...
    const columns: DataTableColumn<ViewOpenTicketsData>[] = [
        {
            accessor: 'actions',
            title: 'Actions',
    ...
    const handleRowClick = (row: ViewOpenTicketsData) => {
            ...
            <ol className="flex text-gray-500 font-semibold dark:text-white-dark flex-wrap">
                ...
                <li
                    className={`before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                        currentPath === '/AdminModule/AdminTickets/OpenTickets/ViewOpenTickets' ? 'text-blue-600' : ''
                    }`}
                >
                    Open Tickets
                </li>
