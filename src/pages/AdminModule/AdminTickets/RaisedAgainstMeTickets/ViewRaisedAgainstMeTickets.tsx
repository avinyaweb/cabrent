...
interface ViewRaisedAgainstMeTicketsData {
    id: string;
    ticketIdKey: string;
    ticketType: string;
    title: string;
...
const ViewRaisedAgainstMeTickets = () => {
    ...
    const [adminTicketsData, setAdminTicketsData] = useState<ViewRaisedAgainstMeTicketsData[]>(staticTicketData);
    const [page, setPage] = useState(1);
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [pageSize, setPageSize] = useState(PAGE_SIZES[0]);
    const [initialRecords, setInitialRecords] = useState<ViewRaisedAgainstMeTicketsData[]>([]);
    const [recordsData, setRecordsData] = useState<ViewRaisedAgainstMeTicketsData[]>([]);
    const [selectedRecords, setSelectedRecords] = useState<ViewRaisedAgainstMeTicketsData[]>([]);
    ...
    useEffect(() => {
        if (adminTicketsData.length > 0) {
            const sortedData = adminTicketsData.slice().sort((a, b) => {
                const accessor = sortStatus.columnAccessor as keyof ViewRaisedAgainstMeTicketsData;
    ...
    const columns: DataTableColumn<ViewRaisedAgainstMeTicketsData>[] = [
        {
            accessor: 'actions',
            title: 'Actions',
    ...
    const handleRowClick = (row: ViewRaisedAgainstMeTicketsData) => {
            ...
            <ol className="flex text-gray-500 font-semibold dark:text-white-dark flex-wrap">
                ...
                <li
                    className={`before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                        currentPath === '/AdminModule/AdminTickets/ReopenTickets/ViewReopenTickets' ? 'text-blue-600' : ''
                    }`}
                >
                    Reopen Tickets
                </li>
...
export default ViewRaisedAgainstMeTickets;
