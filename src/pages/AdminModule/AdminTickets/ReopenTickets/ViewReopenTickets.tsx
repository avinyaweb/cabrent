...
interface ViewReopenTicketsData {
    id: string;
    ticketIdKey: string;
    ticketType: string;
    title: string;
...
const ViewReopenTickets = () => {
    ...
    const [adminTicketsData, setAdminTicketsData] = useState<ViewReopenTicketsData[]>(staticTicketData);
    const [page, setPage] = useState(1);
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [pageSize, setPageSize] = useState(PAGE_SIZES[0]);
    const [initialRecords, setInitialRecords] = useState<ViewReopenTicketsData[]>([]);
    const [recordsData, setRecordsData] = useState<ViewReopenTicketsData[]>([]);
    const [selectedRecords, setSelectedRecords] = useState<ViewReopenTicketsData[]>([]);
    ...
    useEffect(() => {
        if (adminTicketsData.length > 0) {
            const sortedData = adminTicketsData.slice().sort((a, b) => {
                const accessor = sortStatus.columnAccessor as keyof ViewReopenTicketsData;
    ...
    const columns: DataTableColumn<ViewReopenTicketsData>[] = [
        {
            accessor: 'actions',
            title: 'Actions',
    ...
    const handleRowClick = (row: ViewReopenTicketsData) => {
...
export default ViewReopenTickets;
