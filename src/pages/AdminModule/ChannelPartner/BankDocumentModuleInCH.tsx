...
const BankDocumentModuleInCH: React.FC<BankAccountProps> = ({ details, onInputChange, showStatus = true, viewSpecific }) => {
    ...
    const BankPBImages = ['https://rightma.com/wp-content/uploads/2023/02/PNB.png'];
    const cancelledCQImages = ['https://corpseeds.blob.core.windows.net/corpseed/Cancelled_Cheque.png'];
        ...
        <>
            <div>
                <div className="grid grid-cols-1 sm:flex justify-between items-start gap-5 mt-6 mb-6">
                    ...
                    <CarouselModel event={modal6} closeModal={() => setModal6(false)} title={'Bank Passbook'} files={BankPBImages} />
                    {/* ==== */}
                    ...
                    {viewSpecific ? (
                        <div className="lg:w-1/2">
                            ...
                            {cancelledCQImages?.map((imgUrl, ind) => {
                                return (
                                    <img
                                        key={ind}
                                        src={imgUrl}
                                        alt="cancelledCQImages"
                                        className="w-1/3 object-cover cursor-pointer rounded-md hover:scale-105 transition duration-300 shadow-md   border "
                    ...
                    <CarouselModel event={modal5} closeModal={() => setModal5(false)} title={'Cancelled Cheque'} files={cancelledCQImages} />
                </div>
