...
export const staticCouponMasterData = [
    {
        id: '1',
        couponCode: 'SAVE20',
    ...
    },
    {
        ...
        couponDesc: 'Enjoy free shipping on all orders',
        usage: 'Unlimited use',
        amount: '0',
...
interface ViewCouponMasterProps {
    tabs: boolean; // Assuming tabs is supposed to be a boolean
}

const ViewCouponMaster: React.FC<ViewCouponMasterProps> = ({ tabs }) => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const isRtl = useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl' ? true : false;

    const [modal3, setmodal3] = useState(false);
    const [ChannelPartnerData, setChannelPartnerData] = useState<CouponMasterData[]>(staticCouponMasterData);
    const [page, setPage] = useState(1);
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [pageSize, setPageSize] = useState(PAGE_SIZES[0]);
    const [initialRecords, setInitialRecords] = useState<CouponMasterData[]>([]);
    const [recordsData, setRecordsData] = useState<CouponMasterData[]>([]);
    const [selectedRecords, setSelectedRecords] = useState<CouponMasterData[]>([]);
    ...
    // Dynamic Data
    // useEffect(() => {
    //     dispatch(setPageTitle('View Coupon Master'));
    ...
    //                 setChannelPartnerData(staticCouponMasterData);
    //             }
    //         } catch (error: any) {
    //             console.error('Error fetching Coupon Master data:', error.message);
    //         }
    //     };
    ...
    useEffect(() => {
        if (ChannelPartnerData.length > 0) {
            const sortedData = ChannelPartnerData.slice().sort((a, b) => {
                const accessor = sortStatus.columnAccessor as keyof CouponMasterData;
    ...
    const columns: DataTableColumn<CouponMasterData>[] = [
        {
            accessor: 'actions',
            title: 'Actions',
                ...
                <div className="flex items-center">
                        ...
                        <button
                            ...
                            onClick={() => {
                                if (rowData && rowData.id) {
                                    const editUrl = `/PromotionModule/CouponMaster/EditCouponMaster/${rowData.id}`;
                    ...
                    <Tippy content="View Specific">
                        <button
                            ...
                            onClick={() => {
                                if (rowData && rowData.id) {
                                    const viewUrl = `/PromotionModule/CouponMaster/ViewSpecificCouponMaster/${rowData.id}`;
                                    navigate(viewUrl);
                                }
                            }}
                        >
        ...
        { accessor: 'couponCode', title: 'Coupon Code', sortable: true, hidden: hiddenColumns.includes('couponCode') },
    ...
    const handleRowClick = (row: CouponMasterData) => {
        const isSelected = selectedRecords.some((selectedRow) => selectedRow.id === row.id);

        let updatedSelectedRecords: CouponMasterData[] = [];
    ...
    const handleSelectChange = (selectedOption: string) => {
        if (selectedOption === 'edit' && selectedRecords.length === 1) {
            const editUrl = `/PromotionModule/CouponMaster/EditCouponMaster/${selectedRecords[0].id}`;
            navigate(editUrl);
        } else if (selectedOption === 'updateArchive' && selectedRecords.length >= 1) {
            setmodal3(true);
        } else if (selectedOption === 'export' && selectedRecords.length >= 1) {
            downloadExcel(selectedRecords, 'Coupon Master');
        } else {
            ...
            <div className="panel mt-6">
                <div className="grid grid-cols-1 sm:flex justify-between gap-5">
                    <div className="lg:w-1/4 sm:w-full mb-4 sm:mb-0">
                        {tabs ? null : (
                            <Link to="/PromotionModule/CouponMaster/CreateCouponMaster" className="btn btn-primary block w-full sm:inline-block text-center mt-0">
...
export default ViewCouponMaster;
