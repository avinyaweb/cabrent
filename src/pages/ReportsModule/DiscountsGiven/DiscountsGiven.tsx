...
const DiscountsGiven = () => {
    ...
    //         if (data?.discountCounts) {
    //           const filteredData = data.discountCounts.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.discountCounts) {
    //                 const newData = data.discountCounts.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', name = '', discountCount = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || name?.toLowerCase().includes(searchString) || discountCount?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'discountCount', title: 'Discount Count', sortable: true, hidden: hiddenColumns.includes('discountCount') },
    ];
    ...
    const handleSelectChange = (selectedOption: string) => {
        if (selectedOption === 'edit' && selectedRecords.length === 1) {
            ...
            downloadExcel(selectedRecords, 'Discount Given Count');
    ...
    const navItems = [
        ...
        {
            label: 'DiscountsGiven',
            to: '/UtilityModule/Priority/DiscountsGiven',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/DiscountsGiven' ? 'text-blue-600' : ''
            }`,
        },
    ];
