...
const staticPriorityData: PriorityData[] = [
    {
        id: '1',
        totalAiRecognition: '12',
        driverId: 'Dinakar',
    },
    {
        id: '2',
        totalAiRecognition: '33',
        driverId: 'Afsal',
    },
    {
        id: '3',
        totalAiRecognition: '16',
        driverId: 'Mahin',
    },
    // Add more dummy data as needed
];

const TotalAiRecognitions = () => {
    ...
    //         if (data?.driverIds) {
    //           const filteredData = data.driverIds.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.driverIds) {
    //                 const newData = data.driverIds.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', totalAiRecognition = '', driverId = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || totalAiRecognition?.toLowerCase().includes(searchString) || driverId?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'totalAiRecognition', title: 'Total Ai Recognition', sortable: true, hidden: hiddenColumns.includes('totalAiRecognition') },
    ...
    const navItems = [
        ...
        {
            label: 'Total Ai Recognitions',
            to: '/UtilityModule/Priority/TotalAiRecognitions',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalAiRecognitions' ? 'text-blue-600' : ''
            }`,
        },
    ];
...
export default TotalAiRecognitions;
