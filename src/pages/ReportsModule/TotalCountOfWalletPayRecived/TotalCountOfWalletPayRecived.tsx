...
const TotalCountOfWalletPayRecived = () => {
    ...
    //         if (data?.payCounts) {
    //           const filteredData = data.payCounts.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.payCounts) {
    //                 const newData = data.payCounts.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', name = '', payCount = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || name?.toLowerCase().includes(searchString) || payCount?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'payCount', title: 'Wallet Pay Recived Count', sortable: true, hidden: hiddenColumns.includes('payCount') },
    ];
    ...
    const navItems = [
        ...
        {
            label: 'Total Count Of Wallet Pay Recived',
            to: '/UtilityModule/Priority/TotalCountOfWalletPayRecived',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalCountOfWalletPayRecived' ? 'text-blue-600' : ''
            }`,
        },
    ];
...
export default TotalCountOfWalletPayRecived;
