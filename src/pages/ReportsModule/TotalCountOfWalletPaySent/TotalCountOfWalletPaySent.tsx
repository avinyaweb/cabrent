...
const TotalCountOfWalletPaySent = () => {
    ...
    //         if (data?.sentCounts) {
    //           const filteredData = data.sentCounts.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.sentCounts) {
    //                 const newData = data.sentCounts.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', name = '', sentCount = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || name?.toLowerCase().includes(searchString) || sentCount?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'sentCount', title: 'Wallet Pay Sent Count', sortable: true, hidden: hiddenColumns.includes('sentCount') },
    ];
    ...
    const navItems = [
        ...
        {
            label: 'Total Count Of Wallet Pay Sent',
            to: '/UtilityModule/Priority/TotalCountOfWalletPaySent',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalCountOfWalletPaySent' ? 'text-blue-600' : ''
            }`,
        },
    ];
...
export default TotalCountOfWalletPaySent;
