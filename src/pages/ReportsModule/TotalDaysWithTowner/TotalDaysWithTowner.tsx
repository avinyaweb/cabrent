...
const staticPriorityData: PriorityData[] = [
    {
        id: '1',
        totalDaysWithTowner: '233',
        TravelAgencyName: 'Subash limited',
    },
    {
        id: '2',
        totalDaysWithTowner: '123',
        TravelAgencyName: 'anus agency',
    },
    {
        id: '3',
        totalDaysWithTowner: '221',
        TravelAgencyName: 'shajon taxi',
    },
    // Add more dummy data as needed
];

const TotalDaysWithTowner = () => {
    ...
    //         if (data?.TravelAgencyNames) {
    //           const filteredData = data.TravelAgencyNames.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.TravelAgencyNames) {
    //                 const newData = data.TravelAgencyNames.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', totalDaysWithTowner = '', TravelAgencyName = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || totalDaysWithTowner?.toLowerCase().includes(searchString) || TravelAgencyName?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'totalDaysWithTowner', title: 'Total Days With Towner', sortable: true, hidden: hiddenColumns.includes('totalDaysWithTowner') },
        { accessor: 'TravelAgencyName', title: 'Travel Agency Name', sortable: true, hidden: hiddenColumns.includes('TravelAgencyName') },
    ];
    ...
    const navItems = [
        ...
        {
            label: 'Total Days With Towner',
            to: '/UtilityModule/Priority/TotalDaysWithTowner',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalDaysWithTowner' ? 'text-blue-600' : ''
            }`,
        },
    ];
...
export default TotalDaysWithTowner;
