...
const TotalDriverEarned = () => {
    ...
    //         if (data?.totalEarneds) {
    //           const filteredData = data.totalEarneds.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.totalEarneds) {
    //                 const newData = data.totalEarneds.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', driverId = '', totalEarned = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || driverId?.toLowerCase().includes(searchString) || totalEarned?.toLowerCase().includes(searchString);
        });
    ...
    const navItems = [
        ...
        {
            label: 'Total Driver Earned',
            to: '/UtilityModule/Priority/TotalDriverEarned',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalDriverEarned' ? 'text-blue-600' : ''
            }`,
        },
    ];
...
export default TotalDriverEarned;
