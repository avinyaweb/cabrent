...
const staticPriorityData: PriorityData[] = [
    {
        id: '1',
        travelAgencyName: 'Dh limited',
        expence: '1231',
    },
    {
        id: '2',
        travelAgencyName: 'Olija limited',
        expence: '123',
    },
    {
        id: '3',
        travelAgencyName: 'Faqu limited',
        expence: '1234',
    },
...
const TotalExpence = () => {
    ...
    //         if (data?.expences) {
    //           const filteredData = data.expences.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.expences) {
    //                 const newData = data.expences.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', travelAgencyName = '', expence = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || travelAgencyName?.toLowerCase().includes(searchString) || expence?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'travelAgencyName', title: 'Name', sortable: true, hidden: hiddenColumns.includes('travelAgencyName') },
        { accessor: 'expence', title: 'Expence', sortable: true, hidden: hiddenColumns.includes('expence') },
    ];
    ...
    const navItems = [
        ...
        {
            label: 'Total Expence',
            to: '/UtilityModule/Priority/TotalExpence',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalExpence' ? 'text-blue-600' : ''
            }`,
        },
    ];
