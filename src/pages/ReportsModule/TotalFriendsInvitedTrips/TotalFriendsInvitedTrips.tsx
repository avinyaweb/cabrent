...
const TotalFriendsInvitedTrips = () => {
    ...
    //         if (data?.invitedCounts) {
    //           const filteredData = data.invitedCounts.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.invitedCounts) {
    //                 const newData = data.invitedCounts.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', name = '', invitedCount = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || name?.toLowerCase().includes(searchString) || invitedCount?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'invitedCount', title: 'Invited Count', sortable: true, hidden: hiddenColumns.includes('invitedCount') },
    ];
    ...
    const navItems = [
        ...
        {
            label: 'Total Friends Invited Trips',
            to: '/UtilityModule/Priority/TotalFriendsInvitedTrips',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalFriendsInvitedTrips' ? 'text-blue-600' : ''
            }`,
        },
    ];
...
export default TotalFriendsInvitedTrips;
