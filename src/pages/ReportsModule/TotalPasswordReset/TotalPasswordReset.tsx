...
const TotalPasswordReset = () => {
    ...
    //         if (data?.resetCounts) {
    //           const filteredData = data.resetCounts.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.resetCounts) {
    //                 const newData = data.resetCounts.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', name = '', resetCount = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || name?.toLowerCase().includes(searchString) || resetCount?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'resetCount', title: 'Reset Count', sortable: true, hidden: hiddenColumns.includes('resetCount') },
    ];
    ...
    const navItems = [
        ...
        {
            label: 'Total Password Reset',
            to: '/UtilityModule/Priority/TotalPasswordReset',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalPasswordReset' ? 'text-blue-600' : ''
            }`,
        },
    ];
...
export default TotalPasswordReset;
