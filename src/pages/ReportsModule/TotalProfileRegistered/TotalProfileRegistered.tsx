...
const TotalProfileRegistered = () => {
    ...
    //         if (data?.regCounts) {
    //           const filteredData = data.regCounts.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.regCounts) {
    //                 const newData = data.regCounts.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', serviceCity = '', regCount = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || serviceCity?.toLowerCase().includes(searchString) || regCount?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'regCount', title: 'Total Reg Count', sortable: true, hidden: hiddenColumns.includes('regCount') },
    ];
    ...
    const navItems = [
        ...
        {
            label: 'Total Profile Registered',
            to: '/UtilityModule/Priority/TotalProfileRegistered',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalProfileRegistered' ? 'text-blue-600' : ''
            }`,
        },
    ];
...
export default TotalProfileRegistered;
