...
const staticPriorityData: PriorityData[] = [
    {
        id: '1',
        driverId: 'Danil',
        totalRatingRecived: '4',
    },
    {
        id: '2',
        driverId: 'Man',
        totalRatingRecived: '8',
    },
    {
        id: '3',
        driverId: 'John',
...
const TotalRatingRecived = () => {
    ...
    //         if (data?.totalRatingReciveds) {
    //           const filteredData = data.totalRatingReciveds.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.totalRatingReciveds) {
    //                 const newData = data.totalRatingReciveds.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', driverId = '', totalRatingRecived = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || driverId?.toLowerCase().includes(searchString) || totalRatingRecived?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'driverId', title: 'Driver Name', sortable: true, hidden: hiddenColumns.includes('driverId') },
        { accessor: 'totalRatingRecived', title: 'Total Rating Recived', sortable: true, hidden: hiddenColumns.includes('totalRatingRecived') },
    ];
    ...
    const navItems = [
        ...
        {
            label: 'Total Rating Recived',
            to: '/UtilityModule/Priority/TotalRatingRecived',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalRatingRecived' ? 'text-blue-600' : ''
            }`,
        },
    ];
...
export default TotalRatingRecived;
