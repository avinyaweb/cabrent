...
const staticPriorityData: PriorityData[] = [
    {
        id: '1',
        name: 'Disnesh',
        totalWalletAddedAmount: '234',
    },
    {
        id: '2',
        name: 'Brijesh',
        totalWalletAddedAmount: '235',
    },
    {
        id: '3',
        name: 'Sumesh',
        totalWalletAddedAmount: '224',
    },
    // Add more dummy data as needed
];

const TotalWalletAddedAmount = () => {
    ...
    //         if (data?.totalWalletAddedAmounts) {
    //           const filteredData = data.totalWalletAddedAmounts.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.totalWalletAddedAmounts) {
    //                 const newData = data.totalWalletAddedAmounts.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', name = '', totalWalletAddedAmount = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || name?.toLowerCase().includes(searchString) || totalWalletAddedAmount?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'totalWalletAddedAmount', title: 'Total Wallet Added Amount', sortable: true, hidden: hiddenColumns.includes('totalWalletAddedAmount') },
    ];
    ...
    const navItems = [
        ...
        {
            label: 'Total Wallet Added Amount',
            to: '/UtilityModule/Priority/TotalWalletAddedAmount',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalWalletAddedAmount' ? 'text-blue-600' : ''
            }`,
        },
    ];
...
export default TotalWalletAddedAmount;
