...
interface PriorityData {
    id: string;
    name: string;
    totalWalletBonusRecived: string;
}
...
const staticPriorityData: PriorityData[] = [
    {
        id: '1',
        name: 'Disnesh',
        totalWalletBonusRecived: '234',
    },
    {
        id: '2',
        name: 'Brijesh',
        totalWalletBonusRecived: '235',
    },
    {
        id: '3',
        name: 'Sumesh',
        totalWalletBonusRecived: '224',
    },
    // Add more dummy data as needed
];

const TotalWalletBonusRecived = () => {
    ...
    //         if (data?.totalWalletBonusReciveds) {
    //           const filteredData = data.totalWalletBonusReciveds.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.totalWalletBonusReciveds) {
    //                 const newData = data.totalWalletBonusReciveds.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', name = '', totalWalletBonusRecived = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || name?.toLowerCase().includes(searchString) || totalWalletBonusRecived?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'totalWalletBonusRecived', title: 'Total Wallet Bonus Recived', sortable: true, hidden: hiddenColumns.includes('totalWalletBonusRecived') },
    ];
    ...
    const navItems = [
        ...
        {
            label: 'Total Wallet Bonus Recived',
            to: '/UtilityModule/Priority/TotalWalletBonusRecived',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalWalletBonusRecived' ? 'text-blue-600' : ''
            }`,
        },
    ];
...
export default TotalWalletBonusRecived;
