...
const TotalWorkingHours = () => {
    ...
    //         if (data?.totalWorkHourss) {
    //           const filteredData = data.totalWorkHourss.filter((item: PriorityData) => {
    ...
    //             const { data } = await getAllPriority();
    //             if (data?.totalWorkHourss) {
    //                 const newData = data.totalWorkHourss.map(({ _id: id, ...rest }) => ({
    //                     id,
    //                     ...rest,
    //                 }));
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', driverId = '', totalWorkHours = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || driverId?.toLowerCase().includes(searchString) || totalWorkHours?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<PriorityData>[] = [
        ...
        { accessor: 'totalWorkHours', title: 'Total Work Hours', sortable: true, hidden: hiddenColumns.includes('totalWorkHours') },
    ];
    ...
    const navItems = [
        ...
        },
        {
            ...
            to: '/UtilityModule/Priority/TotalWorkingHours',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/UtilityModule/Priority/TotalWorkingHours' ? 'text-blue-600' : ''
            }`,
        },
    ];
