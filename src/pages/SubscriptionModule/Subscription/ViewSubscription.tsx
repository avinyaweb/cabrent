...
const ViewSubscription = ({ tabs, viewCategory }: any) => {
                ...
                <div className="flex items-center">
                    <Tippy content="Edit">
                        <button
                            ...
                            onClick={() => {
                                if (rowData?.id) {
                                    const editUrl = `/SubscriptionModule/Subscription/EditSubscription/${rowData.id}`;
                                    navigate(editUrl);
                                }
                            }}
                        >
    ...
    const handleSelectChange = (selectedOption: string) => {
        if (selectedOption === 'edit' && selectedRecords.length === 1) {
            const editUrl = `/SubscriptionModule/Subscription/EditSubscription/${selectedRecords[0].id}`;
            navigate(editUrl);
        } else if (selectedOption === 'delete' && selectedRecords.length === 1) {
            const confirmDelete = window.confirm('Do you really want to delete this Subscription?');
        ...
        } else if (selectedOption === 'subAmtDist' && selectedRecords.length === 1) {
            setModal4(true);
        } else if (selectedOption === 'AssignSubToVehicle') {
            setAddVehicle(true);
        }
    };
            ...
            <div className="panel mt-6">
                <div className="grid grid-cols-1 sm:flex justify-between gap-5">
                    {/* <div className="lg:w-1/4 sm:w-full mb-4 sm:mb-0">
                        {!viewCategory &&
                            (!tabs ? (
                                <Link to="/SubscriptionModule/Subscription/CreateSubscription" className="btn btn-primary block w-full sm:inline-block text-center mt-0">
                                    Create Subscription
                                </Link>
                            ) : (
                    ...
                    <div className="lg:w-1/4 sm:w-full mb-4 sm:mb-0">
                        <select id="ctnSelect1" className="form-select text-white-dark" onChange={(e) => handleSelectChange(e.target.value)} required>
                            ...
                            <option value="AssignSubToVehicle">Assign Subscription</option>
                            <option value="subAmtDist">Change Amount Distribution</option>
                ...
                />
                <CommonPopUp
                    title={'Assign Subscription To vehicle'}
                    columns={vehicleColumns}
                    data={staticVehicleProfileData}
                    event={AddVehicle}
                    closeModal={() => setAddVehicle(false)}
                    onSubmit={handleAddVehicle}
                />
                <CommonPopUp
                    title={'Change Subscription Amount Distribution'}
                    columns={subscriptionAmountDistColumns}
                    data={staticSubscriptionAmtDistributionData}
                    event={modal4}
                    closeModal={() => setModal4(false)}
                    onSubmit={handleAddSubscriptionAmountDist}
                />
