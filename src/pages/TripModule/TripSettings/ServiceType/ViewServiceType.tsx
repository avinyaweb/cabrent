...
// Define static data
export const staticServiceTypeData: ServiceTypeData[] = [
    {
        id: '1',
        country: 'India',
        state: 'Jharkhand',
...
const ViewServiceType = () => {
    ...
    const [ServiceTypeData, setServiceTypeData] = useState<ServiceTypeData[]>(staticServiceTypeData);
    const [page, setPage] = useState(1);
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [pageSize, setPageSize] = useState(PAGE_SIZES[0]);
    const [initialRecords, setInitialRecords] = useState<ServiceTypeData[]>([]);
    const [recordsData, setRecordsData] = useState<ServiceTypeData[]>([]);

    const [selectedRecords, setSelectedRecords] = useState<ServiceTypeData[]>([]);
    ...
    //         try {
    //             const { data } = await getAllServiceTypeData();
    ...
    useEffect(() => {
        if (ServiceTypeData.length > 0) {
            const sortedData = ServiceTypeData.slice().sort((a, b) => {
                const accessor = sortStatus.columnAccessor as keyof ServiceTypeData;
    ...
    }, [ServiceTypeData, sortStatus, pageSize]);
    ...
    const columns: DataTableColumn<ServiceTypeData>[] = [
        {
            accessor: 'actions',
            title: 'Actions',
                ...
                <div className="flex items-center">
                        ...
                        <button
                            ...
                            onClick={() => {
                                if (rowData && rowData.id) {
                                    const editUrl = `/UtilityModule/ServiceType/EditServiceType/${rowData.id}`;
                    ...
                    <Tippy content="View Specific">
                        <button
                            ...
                            onClick={() => {
                                if (rowData && rowData.id) {
                                    const viewUrl = `/UtilityModule/ServiceType/ViewSpecificServiceType/${rowData.id}`;
                                    navigate(viewUrl);
                                }
                            }}
                        >
    ...
    const handleRowClick = (row: ServiceTypeData) => {
        const isSelected = selectedRecords.some((selectedRow) => selectedRow.id === row.id);

        let updatedSelectedRecords: ServiceTypeData[] = [];
            ...
            <div className="panel mt-6">
                <div className="grid grid-cols-1 sm:flex justify-between gap-5">
                    <div className="lg:w-1/4 sm:w-full mb-4 sm:mb-0">
                        <Link to="/TripModule/TripSettings/ServiceType/CreateServiceType" className="btn btn-primary block w-full sm:inline-block text-center mt-0">
                            Create Service Type
                        </Link>
                    </div>
                ...
                <div className="datatables mt-6">
                    <DataTable
                        ...
                        columns={columns}
                        highlightOnHover
                        totalRecords={ServiceTypeData.length}
