...
const ViewCancelledTrips = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const isRtl = useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl' ? true : false;

    const [CancelledTripsData, setCancelledTripsData] = useState<TripsData[]>(staticTripsData);
    ...
    useEffect(() => {
        ...
        // future code --->>>
        // const fetchCancelledTripsData = async () => {
        //     try {
        //         const { data } = await getCancelledTripsData();
        //         if (data?.CancelledTripss) {
        //             // pk-note: ask backend developer to change the accessor _id:id, remove the following later
        //             const newData = data.CancelledTripss.map(({ _id: id, ...rest }: { _id: string; [key: string]: any }) => ({
        //                 id,
        //                 ...rest,
        //             }));
        //             //setCancelledTripsData(newData);
        //             setCancelledTripsData(staticTripsData);
        //         }
        //     } catch (error: any) {
        //         console.error('Error fetching Trips data:', error.message);
        //     }
        // };
        // fetchCancelledTripsData();
    }, [dispatch]);

    useEffect(() => {
        if (CancelledTripsData.length > 0) {
            const sortedData = CancelledTripsData.slice().sort((a, b) => {
    ...
    }, [CancelledTripsData, sortStatus, pageSize]);
    ...
    const handleRowClick = (row: TripsData) => {
        ...
        navigate(`/AdminModule/CancelledTrips/ViewSpecificCancelledTrips/${row.id}`);
    };
            ...
            <div className="panel mt-6">
                ...
                <div className="datatables mt-6">
                    <DataTable
                        ...
                        columns={columns}
                        highlightOnHover
                        totalRecords={CancelledTripsData.length}
...
export default ViewCancelledTrips;
