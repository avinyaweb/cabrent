...
const ViewNoResponse = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const isRtl = useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl' ? true : false;

    const [NoResponseData, setNoResponseData] = useState<TripsData[]>(staticTripsData);
    ...
    useEffect(() => {
        ...
        //     try {
        //         const { data } = await getNoResponseData();
        //         if (data?.NoResponses) {
        //             // pk-note: ask backend developer to change the accessor _id:id, remove the following later
        //             const newData = data.NoResponses.map(({ _id: id, ...rest }: { _id: string; [key: string]: any }) => ({
        //                 id,
        //                 ...rest,
        //             }));
        //             //setNoResponseData(newData);
        //             setNoResponseData(staticTripsData);
        //         }
        //     } catch (error: any) {
    ...
    useEffect(() => {
        if (NoResponseData.length > 0) {
            const sortedData = NoResponseData.slice().sort((a, b) => {
    ...
    }, [NoResponseData, sortStatus, pageSize]);
    ...
    const handleRowClick = (row: TripsData) => {
        ...
        navigate(`/AdminModule/NoResponse/ViewSpecificNoResponse/${row.id}`);
    };
            ...
            <div className="panel mt-6">
                ...
                <div className="datatables mt-6">
                    <DataTable
                        ...
                        columns={columns}
                        highlightOnHover
                        totalRecords={NoResponseData.length}
