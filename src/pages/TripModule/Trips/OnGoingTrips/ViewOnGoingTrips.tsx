...
const ViewOnGoingTrips = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const isRtl = useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl' ? true : false;

    const [OnGoingTripsData, setOnGoingTripsData] = useState<TripsData[]>(staticTripsData);
    ...
    useEffect(() => {
        ...
        // future code --->>>
        // const fetchOnGoingTripsData = async () => {
        //     try {
        //         const { data } = await getOnGoingTripsData();
        //         if (data?.OnGoingTripss) {
        //             // pk-note: ask backend developer to change the accessor _id:id, remove the following later
        //             const newData = data.OnGoingTripss.map(({ _id: id, ...rest }: { _id: string; [key: string]: any }) => ({
        //                 id,
        //                 ...rest,
        //             }));
        //             //setOnGoingTripsData(newData);
        //             setOnGoingTripsData(staticTripsData);
        //         }
        //     } catch (error: any) {
        //         console.error('Error fetching Trips data:', error.message);
        //     }
        // };
        // fetchOnGoingTripsData();
    }, [dispatch]);

    useEffect(() => {
        if (OnGoingTripsData.length > 0) {
            const sortedData = OnGoingTripsData.slice().sort((a, b) => {
    ...
    }, [OnGoingTripsData, sortStatus, pageSize]);
    ...
    const handleRowClick = (row: TripsData) => {
        ...
        navigate(`/AdminModule/OnGoingTrips/ViewSpecificOnGoingTrips/${row.id}`);
    };
            ...
            <div className="panel mt-6">
                ...
                <div className="datatables mt-6">
                    <DataTable
                        ...
                        columns={columns}
                        highlightOnHover
                        totalRecords={OnGoingTripsData.length}
...
export default ViewOnGoingTrips;
