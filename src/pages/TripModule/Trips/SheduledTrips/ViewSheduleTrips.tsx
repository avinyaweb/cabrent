...
const ViewSheduleTrips = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const isRtl = useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl' ? true : false;

    const [SchedulesTripData, setSchedulesTripData] = useState<TripsData[]>(staticTripsData);
    ...
    useEffect(() => {
        ...
        // future code --->>>
        // const fetchSchedulesTripData = async () => {
        //     try {
        //         const { data } = await getSchedulesTripData();
        //         if (data?.SchedulesTrips) {
        //             // pk-note: ask backend developer to change the accessor _id:id, remove the following later
        //             const newData = data.SchedulesTrips.map(({ _id: id, ...rest }: { _id: string; [key: string]: any }) => ({
        //                 id,
        //                 ...rest,
        //             }));
        //             //setSchedulesTripData(newData);
        //             setSchedulesTripData(staticTripsData);
        //         }
        //     } catch (error: any) {
        //         console.error('Error fetching Trips data:', error.message);
        //     }
        // };
        // fetchSchedulesTripData();
    }, [dispatch]);

    useEffect(() => {
        if (SchedulesTripData.length > 0) {
            const sortedData = SchedulesTripData.slice().sort((a, b) => {
    ...
    }, [SchedulesTripData, sortStatus, pageSize]);
    ...
    const handleRowClick = (row: TripsData) => {
        ...
        navigate(`/AdminModule/SchedulesTrip/ViewSpecificSchedulesTrip/${row.id}`);
    };
            ...
            <div className="panel mt-6">
                ...
                <div className="datatables mt-6">
                    <DataTable
                        ...
                        columns={columns}
                        highlightOnHover
                        totalRecords={SchedulesTripData.length}
...
export default ViewSheduleTrips;
