...
import { getAllCompanyTypeById } from '@/services/UtilityServices/CompanyTypeService';
...
const ViewSpecificCompanyType: React.FC = () => {
    const { companyTypeId } = useParams<{ companyTypeId?: string }>();
    ...
    useEffect(() => {
        const fetchChannelPartnerDetails = async () => {
            if (companyTypeId) {
                // Ensure `companyTypeId` is defined before making the API call
                try {
                    const response = await getAllCompanyTypeById(companyTypeId);
                    console.log('Fetched Data', response);
                    setCompanyTypeDetails(response.data.CompanyType);
                ...
                console.error('No companyTypeId provided');
            }
        };
    ...
    const navItems = [
        ...
        {
            label: 'View Company Type',
            to: '/UtilityModule/CompanyType/ViewSpecificCompanyType',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === `/UtilityModule/CompanyType/ViewSpecificCompanyType/${companyTypeId}` ? 'text-blue-600' : ''
            }`,
        },
    ];
            ...
            <div className="panel mt-6">
                <Tab.Group>
                    <Tab.List className="mt-3 flex flex-wrap border-b border-white-light dark:border-[#191e3a]">
                        <Tab as={Fragment}>
                            {({ selected }) => (
                                ...
                                >
                                    <LuBuilding2 className="w-5 h-5 ltr:mr-2 rtl:ml-2" />
                                    <span className="text-md font-bold">Company Type</span>
                                </button>
                            )}
                        </Tab>
                        <Tab as={Fragment}>
                    ...
                    <Tab.Panels>
                        <Tab.Panel>
                            <div className="mt-5">
                                <CompanyTypeModule details={companyTypeDetails} onInputChange={handleInputChange} showStatus={true} viewSpecific={viewSpecific} />
                                ...
                                <div className="grid grid-cols-1 sm:flex justify-between gap-5 mt-6">
                                    <div className="lg:w-1/3">
                                        ...
                                        <input
                                            ...
                                            // value={companyTypeDetails?.approvedAt}
                                    ...
                                    <div className="lg:w-1/3">
                                        ...
                                        <input
                                            ...
                                            className={`form-input w-full ${viewSpecific ? 'pointer-events-none' : ''}`}
                                            readOnly={viewSpecific}
                                            // value={companyTypeDetails?.approvedBy}
                                        />
                                    </div>
                                    <div className="lg:w-1/3">
                                        ...
                                        <input
                                            ...
                                            className={`form-input w-full ${viewSpecific ? 'pointer-events-none' : ''}`}
                                            readOnly={viewSpecific}
                                            // value={companyTypeDetails?.createdAt}
                                        />
                                    </div>
                                    <div className="lg:w-1/3">
                                        ...
                                        <input
                                            ...
                                            className={`form-input w-full ${viewSpecific ? 'pointer-events-none' : ''}`}
                                            readOnly={viewSpecific}
                                            // value={companyTypeDetails?.createdBy}
                                        />
                                    </div>
                                </div>
                            </div>
...
export default ViewSpecificCompanyType;
