...
const staticData: StateData[] = [
    {
        ...
        SecCurrencyExists: 'Yes',
        SecondaryCurrencyName: 'EUR',
        SecondaryCurrencySymbol: '$',
        CurrencyConversionMultipleRate: '1.2',
    },
    {
        id: '2',
        PrimaryCurrencyName: 'GBP',
        SecCurrencyExists: 'No',
        SecondaryCurrencyName: 'USD',
        SecondaryCurrencySymbol: '£',
        CurrencyConversionMultipleRate: '1.4',
    },
    {
        id: '3',
        PrimaryCurrencyName: 'EUR',
        SecCurrencyExists: 'Yes',
        SecondaryCurrencyName: 'GBP',
        SecondaryCurrencySymbol: '€',
        CurrencyConversionMultipleRate: '0.8',
    },
];
...
const ViewCancellationReasons = () => {
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', PrimaryCurrencyName = '', SecCurrencyExists = '', SecondaryCurrencyName = '', SecondaryCurrencySymbol = '', CurrencyConversionMultipleRate = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return (
                id?.toLowerCase().includes(searchString) ||
                PrimaryCurrencyName?.toLowerCase().includes(searchString) ||
                SecCurrencyExists?.toLowerCase().includes(searchString) ||
                SecondaryCurrencyName?.toLowerCase().includes(searchString) ||
                SecondaryCurrencySymbol?.toLowerCase().includes(searchString) ||
                CurrencyConversionMultipleRate?.toLowerCase().includes(searchString)
            );
        });
    ...
    const columns: DataTableColumn<StateData>[] = [
                ...
                <div className="flex items-center">
                        ...
                        <button
                            ...
                            onClick={() => {
                                if (rowData && rowData.id) {
                                    const editUrl = `/UtilityModule/CurrencyManagement/EditCurrency/${rowData.id}`;
                    ...
                    <Tippy content="View Specific">
                        <button
                            ...
                            onClick={() => {
                                if (rowData && rowData.id) {
                                    const viewUrl = `/UtilityModule/CurrencyManagement/ViewSpecificCurrency/${rowData.id}`;
                                    navigate(viewUrl);
                                }
                            }}
                        >
        ...
        { accessor: 'PrimaryCurrencyName', title: 'Primary Currency Name', sortable: true, hidden: hiddenColumns.includes('PrimaryCurrencyName') },
        { accessor: 'SecCurrencyExists', title: 'Sec Currency Exists', sortable: true, hidden: hiddenColumns.includes('SecCurrencyExists') },
        { accessor: 'SecondaryCurrencyName', title: 'Secondary Currency Name', sortable: true, hidden: hiddenColumns.includes('SecondaryCurrencyName') },
        { accessor: 'SecondaryCurrencySymbol', title: 'Secondary Currency Symbol', sortable: true, hidden: hiddenColumns.includes('SecondaryCurrencySymbol') },
        { accessor: 'CurrencyConversionMultipleRate', title: 'Currency Conversion Multiple Rate', sortable: true, hidden: hiddenColumns.includes('CurrencyConversionMultipleRate') },
    ];
            ...
            <div className="panel mt-6">
                <div className="grid grid-cols-1 sm:flex justify-between gap-5">
                    <div className="lg:w-1/4 sm:w-full mb-4 sm:mb-0">
                        <Link to="/utilityModule/CurrencyManagement/CreateCurrencyData" className="btn btn-primary block w-full sm:inline-block text-center mt-0">
                            Create Currency
                        </Link>
                    </div>
