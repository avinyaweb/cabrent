...
const staticData: StateData[] = [
    {
        ...
        feedbackReason: ' reason 1',
    },
    {
        id: '2',

        feedbackReason: ' reason 2',
    },
    {
        id: '3',

        feedbackReason: ' reason 3',
    },
];

const ViewFeedbackReasons = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();

    useEffect(() => {
        dispatch(setPageTitle('View feedback reason'));
    }, [dispatch]);
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            const { id = '', feedbackReason = '' } = item || {};

            const searchString = search ? search.toLowerCase() : ''; // Ensure search is defined and convert to lowercase

            return id?.toLowerCase().includes(searchString) || feedbackReason?.toLowerCase().includes(searchString);
        });
    ...
    const columns: DataTableColumn<StateData>[] = [
                ...
                <div className="flex items-center">
                        ...
                        <button
                            ...
                            onClick={() => {
                                if (rowData && rowData.id) {
                                    const editUrl = `/UtilityModule/FeedbackReasons/EditFeedbackReason/${rowData.id}`;
                    ...
                    <Tippy content="View Specific">
                        <button
                            ...
                            onClick={() => {
                                if (rowData && rowData.id) {
                                    const viewUrl = `/UtilityModule/FeedbackReasons/ViewSpecificFeedbackReason/${rowData.id}`;
                                    navigate(viewUrl);
                                }
                            }}
                        >
        ...
        { accessor: 'feedbackReason', title: 'Feedback Reason', sortable: true, hidden: hiddenColumns.includes('feedbackReason') },
    ];
            ...
            <div className="panel mt-6">
                <div className="grid grid-cols-1 sm:flex justify-between gap-5">
                    <div className="lg:w-1/4 sm:w-full mb-4 sm:mb-0">
                        <Link to="/UtilityModule/FeedbackReasons/CreateFeedBackReason" className="btn btn-primary block w-full sm:inline-block text-center mt-0">
                            Create Feedback Reasons
                        </Link>
                    </div>
...
export default ViewFeedbackReasons;
