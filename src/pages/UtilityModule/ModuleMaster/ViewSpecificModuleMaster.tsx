...
const ViewSpecificModuleMaster: React.FC = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();

    // const { moduleMasterId } = useParams();
    const { moduleMasterId } = useParams<{ moduleMasterId?: string }>();
    ...
    //             setModuleMasterDetails(response.data.Module);
    //         } catch (error: any) {
    ...
    useEffect(() => {
        const fetchModuleDetails = async () => {
            if (moduleMasterId) {
                // Ensure `moduleMasterId` is defined before making the API call
                try {
                    const response = await getModuleById(moduleMasterId);
                    setModuleMasterDetails(response.data.Module);
                } catch (error: any) {
                ...
                console.error('No moduleMasterId provided');
            }
        };
    ...
    const navItems = [
        ...
        {
            label: 'View Module Master',
            to: '/UtilityModule/ModuleMaster/ViewSpecificModuleMaster',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === `/UtilityModule/ModuleMaster/ViewSpecificModuleMaster/${moduleMasterId}` ? 'text-blue-600' : ''
            }`,
        },
    ];
            ...
            <div className="panel mt-6">
                <Tab.Group>
                    <Tab.List className="mt-3 flex flex-wrap border-b border-white-light dark:border-[#191e3a]">
                        <Tab as={Fragment}>
                            {({ selected }) => (
                                ...
                                >
                                    <MdViewModule className="w-5 h-5 ltr:mr-2 rtl:ml-2" />
                                    <span className="text-md font-bold">Module Master</span>
                                </button>
                            )}
                        </Tab>
                        <Tab as={Fragment}>
                    ...
                    <Tab.Panels>
                        <Tab.Panel>
                            <div className="mt-5">
                                <ModuleMasterModule details={moduleMasterDetails} onInputChange={handleInputChange} showStatus={true} viewSpecific={true} />
                                ...
                                <div className="grid grid-cols-1 sm:flex justify-between gap-5 mt-6">
                                    <div className="lg:w-1/3">
                                        ...
                                        <input name="approvedAt" type="text" id="approvedAt" className="form-input w-full pointer-events-none" value={moduleMasterDetails.approvedAt} />
                                    </div>
                                    <div className="lg:w-1/3">
                                        <label htmlFor="approvedBy" className="block mb-1">
                                            Approved By
                                        </label>
                                        <input name="approvedBy" type="text" id="approvedBy" className="form-input w-full pointer-events-none" value={moduleMasterDetails.approvedBy} />
                                    </div>
                                    <div className="lg:w-1/3">
                                        <label htmlFor="createdAt" className="block mb-1">
                                            Created At
                                        </label>
                                        <input name="createdAt" type="text" id="createdAt" className="form-input w-full pointer-events-none" value={moduleMasterDetails.createdAt} />
                                    </div>
                                    <div className="lg:w-1/3">
                                        <label htmlFor="createdBy" className="block mb-1">
                                            Created By
                                        </label>
                                        <input name="createdBy" type="text" id="createdBy" className="form-input w-full pointer-events-none" value={moduleMasterDetails.createdBy} />
                                    </div>
                                </div>
                            </div>
                        </Tab.Panel>
...
export default ViewSpecificModuleMaster;
