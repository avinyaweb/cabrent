...
import { getStatusById } from '@/services/UtilityServices/StatusService';
...
const ViewSpecificStatus: React.FC = () => {
    ...
    useEffect(() => {
        const fetchChannelPartnerDetails = async () => {
            try {
                const response = await getStatusById(statusId ?? ''); // Provide default value for statusId
                console.log('Fetched Data', response);
                setstatusDetails(response.data.Archive);
            } catch (error: any) {
                console.error('Error fetching channel partner details:', error.message);
    ...
    const navItems = [
        ...
        {
            label: 'View Status',
            to: '/UtilityModule/Status/ViewSpecificStatus',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === `/UtilityModule/Status/ViewSpecificStatus/${statusId}` ? 'text-blue-600' : ''
            }`,
        },
    ];
            ...
            <div className="panel mt-6">
                <Tab.Group>
                    <Tab.List className="mt-3 flex flex-wrap border-b border-white-light dark:border-[#191e3a]">
                        <Tab as={Fragment}>
                            {({ selected }) => (
                                ...
                                >
                                    <IoArchive className="w-5 h-5 ltr:mr-2 rtl:ml-2" />
                    ...
                    <Tab.Panels>
                        <Tab.Panel>
                            <div className="mt-5">
                                <StatusModule details={statusDetails} onInputChange={handleInputChange} showStatus={true} viewSpecific={true} />
                                ...
                                <div className="grid grid-cols-1 sm:flex justify-between gap-5 mt-6">
                                    <div className="lg:w-1/3">
                                        ...
                                        <input name="approvedAt" type="text" id="approvedAt" className="form-input w-full pointer-events-none" value={'6583d91a720b1dc8eab67725'} />
                                    </div>
                                    <div className="lg:w-1/3">
                                        <label htmlFor="approvedBy" className="block mb-1">
                                            Approved By
                                        </label>
                                        <input name="approvedBy" type="text" id="approvedBy" className="form-input w-full pointer-events-none" value={'6583d91a720b1dc8eab67725'} />
                                    </div>
                                    <div className="lg:w-1/3">
                                        <label htmlFor="createdAt" className="block mb-1">
                                            Created At
                                        </label>
                                        <input name="createdAt" type="text" id="createdAt" className="form-input w-full pointer-events-none" value={'6583d91a720b1dc8eab67725'} />
                                    </div>
                                    <div className="lg:w-1/3">
                                        <label htmlFor="createdBy" className="block mb-1">
                                            Created By
                                        </label>
                                        <input name="createdBy" type="text" id="createdBy" className="form-input w-full pointer-events-none" value={'6583d91a720b1dc8eab67725'} />
                                    </div>
                                </div>
                            </div>
                        </Tab.Panel>
...
export default ViewSpecificStatus;
