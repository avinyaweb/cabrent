...
interface DistributorTransactionData {
    id: string;
    amount: string;
    fk_userid: string;
...
export const staticDistributorTransactionData = [
    {
        id: '1',
        amount: '100.00',
        fk_userid: 'user123',
...
interface ViewDistributorTransactionProps {
    tabs: any; // Adjust the type of tabs according to your requirements
    // Add other props if needed
}

const ViewDistributorTransaction: React.FC<ViewDistributorTransactionProps> = ({ tabs }) => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const isRtl = useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl' ? true : false;
    const [modal3, setmodal3] = useState(false);
    const [DistributorTransactionData, setDistributorTransactionData] = useState<DistributorTransactionData[]>(staticDistributorTransactionData);
    const [page, setPage] = useState(1);
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [pageSize, setPageSize] = useState(PAGE_SIZES[0]);
    const [initialRecords, setInitialRecords] = useState<DistributorTransactionData[]>([]);
    const [recordsData, setRecordsData] = useState<DistributorTransactionData[]>([]);
    const [selectedRecords, setSelectedRecords] = useState<DistributorTransactionData[]>([]);
    ...
    //   const fetchDistributorTransactionData = async () => {
    //     try {
    //       const { data } = await getDistributorTransactionData();
    ...
    //           ...rest
    //         }))
    //         //setDistributorTransactionData(newData);
    //         setDistributorTransactionData(staticDistributorTransactionData);
    //       }
    //     } catch (error: any) {
    //       console.error('Error fetching pg transactions data:', error.message);
    //     }
    //   };
    //   fetchDistributorTransactionData();
    // }, [dispatch]);

    useEffect(() => {
        if (DistributorTransactionData.length > 0) {
            const sortedData = DistributorTransactionData.slice().sort((a, b) => {
                const accessor = sortStatus.columnAccessor as keyof DistributorTransactionData;
    ...
    }, [DistributorTransactionData, sortStatus, pageSize]);
    ...
    const columns: DataTableColumn<DistributorTransactionData>[] = [
        {
            accessor: 'actions',
            title: 'Actions',
    ...
    const handleRowClick = (row: DistributorTransactionData) => {
        const isSelected = selectedRecords.some((selectedRow) => selectedRow.id === row.id);

        let updatedSelectedRecords: DistributorTransactionData[] = [];
    ...
    const navItems = [
        ...
        {
            label: 'Distributor Transaction',
            to: '/TransactionModule/TripTransaction/ViewDistributorTransaction',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/TransactionModule/TripTransaction/ViewDistributorTransaction' ? 'text-blue-600' : ''
            }`,
        },
    ];
            ...
            <div className="panel mt-6">
                ...
                <div className="datatables mt-6">
                    <DataTable
                        ...
                        columns={columns}
                        highlightOnHover
                        totalRecords={DistributorTransactionData.length}
