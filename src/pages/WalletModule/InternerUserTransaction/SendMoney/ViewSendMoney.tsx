...
export const staticSendMoneyData = [
    {
        id: '1',
        amount: '100.00',
        senderDriverid: 'Junaid',
        toDriverid: 'Ramesh kumar',
        transactionMode: 'QR',
        status: 'PENDING',
        approvedBy: 'AdminXYZ',
...
interface ViewSendMoneyProps {
    tabs: any; // Adjust the type of tabs according to your requirements
    // Add other props if needed
}

const ViewSendMoney: React.FC<ViewSendMoneyProps> = ({ tabs }) => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const isRtl = useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl' ? true : false;
    const [modal3, setmodal3] = useState(false);
    const [SendMoneyData, setSendMoneyData] = useState<SendMoneyData[]>(staticSendMoneyData);
    const [page, setPage] = useState(1);
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [pageSize, setPageSize] = useState(PAGE_SIZES[0]);
    const [initialRecords, setInitialRecords] = useState<SendMoneyData[]>([]);
    const [recordsData, setRecordsData] = useState<SendMoneyData[]>([]);
    const [selectedRecords, setSelectedRecords] = useState<SendMoneyData[]>([]);
    ...
    //     try {
    //       const { data } = await getSendMoneyData();
    ...
    //           ...rest
    //         }))
    //         //setSendMoneyData(newData);
    //         setSendMoneyData(staticSendMoneyData);
    //       }
    //     } catch (error: any) {
    ...
    useEffect(() => {
        if (SendMoneyData.length > 0) {
            const sortedData = SendMoneyData.slice().sort((a, b) => {
                const accessor = sortStatus.columnAccessor as keyof SendMoneyData;
    ...
    }, [SendMoneyData, sortStatus, pageSize]);
    ...
    useEffect(() => {
        const filteredData = initialRecords.filter((item) => {
            ...
            return (
                ...
                senderDriverid?.toLowerCase().includes(searchString) ||
                toDriverid?.toLowerCase().includes(searchString) ||
    ...
    const columns: DataTableColumn<SendMoneyData>[] = [
        {
            accessor: 'actions',
            title: 'Actions',
                        ...
                        <button
                            ...
                            onClick={() => {
                                if (rowData && rowData.id) {
                                    const viewUrl = `/WalletModule/InternerUserTransaction/SendMoney/ViewSpecificSendMoney/1`;
                                    navigate(viewUrl);
                                }
                            }}
                        >
        ...
        { accessor: 'senderDriverid', title: 'Sender DriverName', sortable: true, hidden: hiddenColumns.includes('senderDriverid') },
        { accessor: 'toDriverid', title: 'Sendto DriverName ', sortable: true, hidden: hiddenColumns.includes('toDriverid') },
    ...
    const handleRowClick = (row: SendMoneyData) => {
        const isSelected = selectedRecords.some((selectedRow) => selectedRow.id === row.id);

        let updatedSelectedRecords: SendMoneyData[] = [];
    ...
    const navItems = [
        ...
        {
            label: 'Send Money',
            to: '/TransactionModule/TripTransaction/ViewSendMoney',
            className: `before:w-1 before:h-1 before:rounded-full before:bg-primary before:inline-block before:relative before:-top-0.5 before:mx-4 sm:mb-0 whitespace-nowrap sm:whitespace-normal ${
                currentPath === '/TransactionModule/TripTransaction/ViewSendMoney' ? 'text-blue-600' : ''
            }`,
        },
    ];
            ...
            <div className="panel mt-6">
                ...
                <div className="datatables mt-6">
                    <DataTable
                        ...
                        columns={columns}
                        highlightOnHover
                        totalRecords={SendMoneyData.length}
