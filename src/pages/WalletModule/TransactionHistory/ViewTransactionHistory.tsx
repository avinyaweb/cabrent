...
const staticTransactionData: TransactionData[] = [
    {
        id: '1',
        fromUser: 'UserA',
        toUser: 'UserB',
...
const ViewTransactionHistory = () => {
    ...
    const [TransactionData, setTransactionData] = useState<TransactionData[]>(staticTransactionData);
    const [page, setPage] = useState(1);
    const PAGE_SIZES = [10, 20, 30, 50, 100];
    const [pageSize, setPageSize] = useState(PAGE_SIZES[0]);
    const [initialRecords, setInitialRecords] = useState<TransactionData[]>([]);
    const [recordsData, setRecordsData] = useState<TransactionData[]>([]);

    const [selectedRecords, setSelectedRecords] = useState<TransactionData[]>([]);
    ...
    //         if (data?.Archives) {
    //           const filteredData = data.Archives.filter((item: TransactionData) => {
    ...
    useEffect(() => {
        if (TransactionData.length > 0) {
            const sortedData = TransactionData.slice().sort((a, b) => {
                const accessor = sortStatus.columnAccessor as keyof TransactionData;
    ...
    }, [TransactionData, sortStatus, pageSize]);
                ...
                <div className="flex items-center">
                    ...
                    <Tippy content="View Specific">
                        <button
                            ...
                            onClick={() => {
                                if (rowData?.id) {
                                    const viewUrl = `/WalletModule/TransactionHistory/ViewSpecificTransactionHistory/${rowData.id}`;
                                    navigate(viewUrl);
                                }
                            }}
                        >
    ...
    const handleRowClick = (row: TransactionData) => {
        ...
        let updatedSelectedRecords: TransactionData[] = [];
